using System;
using System.Collections.Generic;
using System.Data;
using System.Data.Entity;
using System.Data.Entity.Infrastructure;
using System.Linq;
using System.Net;
using System.Net.Http;
using System.Threading.Tasks;
using System.Web.Http;
using System.Web.Http.Description;
using APIEFSPAserviceStudy.Data;
using APIEFSPAserviceStudy.Models;
using APIEFSPAserviceStudy.DTOs;
using System.Linq.Expressions;
using System.Web.Http.Tracing;

namespace APIEFSPAserviceStudy.Controllers
{
    public class BooksController : ApiController
    {
        private APIEFSPAserviceStudyContext db = new APIEFSPAserviceStudyContext();

        private static readonly Expression<Func<Book, BookDto>> AsBookDto =
            x => new BookDto
            {
                BookId = x.BookId,
                Title = x.Title,
                AuthorName = x.Author.Name
            };

        private static readonly Expression<Func<Book, BookDetailDto>> AsBookDetailDto =
            x => new BookDetailDto
            {
                BookId = x.BookId,
                Title = x.Title,
                Year = x.Year,
                Price = x.Price,
                AuthorName = x.Author.Name,
                Genre = x.Genre
            };

        // GET: api/Books
        public IQueryable<BookDto> GetBooks()
        {
            Configuration.Services.GetTraceWriter().Info(
                Request, "BooksController", "Get the list of Book.");

            return db.Books
                .Include(b => b.Author)
                .Select(AsBookDto);
        }

        // GET: api/Books/5
        [ResponseType(typeof(BookDetailDto))]
        public async Task<IHttpActionResult> GetBook(int id)
        {
            //Book book = await db.Books.FindAsync(id);
            BookDetailDto book = await db.Books
                .Include(b => b.Author)
                .Select(AsBookDetailDto)
                .SingleOrDefaultAsync(b => b.BookId == id);

            if (book == null)
            {
                return NotFound();
            }

            return Ok(book);
        }

        // PUT: api/Books/5
        [ResponseType(typeof(void))]
        public async Task<IHttpActionResult> PutBook(int id, Book book)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            }

            if (id != book.BookId)
            {
                return BadRequest();
            }

            db.Entry(book).State = EntityState.Modified;

            try
            {
                await db.SaveChangesAsync();
            }
            catch (DbUpdateConcurrencyException)
            {
                if (!BookExists(id))
                {
                    return NotFound();
                }
                else
                {
                    throw;
                }
            }

            return StatusCode(HttpStatusCode.NoContent);
        }

        // POST: api/Books
        [ResponseType(typeof(BookDto))]
        public async Task<IHttpActionResult> PostBook(Book book)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            }

            db.Books.Add(book);
            await db.SaveChangesAsync();

            db.Entry(book).Reference(x => x.Author).Load();

            var dto = new BookDto()
            {
                BookId = book.BookId,
                Title = book.Title,
                AuthorName = book.Author.Name
            };

            return CreatedAtRoute("DefaultApi", new { id = book.BookId }, dto);
        }

        // DELETE: api/Books/5
        [ResponseType(typeof(Book))]
        public async Task<IHttpActionResult> DeleteBook(int id)
        {
            Book book = await db.Books.FindAsync(id);
            if (book == null)
            {
                return NotFound();
            }

            db.Books.Remove(book);
            await db.SaveChangesAsync();

            return Ok(book);
        }

        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                db.Dispose();
            }
            base.Dispose(disposing);
        }

        private bool BookExists(int id)
        {
            return db.Books.Count(e => e.BookId == id) > 0;
        }
    }
}